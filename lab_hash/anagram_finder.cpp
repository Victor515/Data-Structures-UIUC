...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
    Dict<char,int> word_ht(256);
    for(auto character: word){
      if(!word_ht.keyExists(character)){
        word_ht.insert(character, 1);
      }
      else{
        word_ht[character]++;
      }
    ...
    Dict<char, int> test_ht(256);
    for(auto character1: test){
      if(!test_ht.keyExists(character1)){
        test_ht.insert(character1, 1);
      }
      else{
        test_ht[character1]++;
      }
    ...
    for(it = word_ht.begin(); it != word_ht.end(); it++){
      // check if char exist in test table
      if(!test_ht.keyExists(it->first)){
        return false;
      }

      // if char exist, check if count is the same
      else{
        if(it->second != test_ht[it->first]){
          return false;
        }
      }
    }
