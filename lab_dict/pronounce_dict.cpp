...
{
    /* Your code goes here! */
    // transform two words to uppercase
    string w1(word1);
    string w2(word2);
    std::transform(w1.begin(), w1.end(), w1.begin(), ::toupper);
    std::transform(w2.begin(), w2.end(), w2.begin(), ::toupper);
    // if word 1 or 2 does not exist in dict
    if(dict.find(w1) == dict.end()|| dict.find(w2) == dict.end() ){
      return false;
    }

    // get syllables of word1 and word2
    std::vector<string> w1_syllables = dict.at(w1);
    std::vector<string> w2_syllables = dict.at(w2);
    if(w1_syllables == w2_syllables){
      return true;
    }
