...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for(std::pair<string, unsigned int>  key_val : common){
      // if the word appears in each file
      if(key_val.second == file_word_maps.size()){
        // check whether it appears in each file >= n times
        string word = key_val.first;
        size_t i = 0;
        for(; i < file_word_maps.size(); i++){
          // if < n times, exit the loop
          if(file_word_maps[i].at(word) < n){
            break;
          }
        }
        // if the word appears in each file >=n times
