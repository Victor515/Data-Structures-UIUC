                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(mergeQueue.empty()){
      // consider the situations when mergeQueue is empty at the start
    ...
    if(singleQueue.empty()){
      // when singleQueue becomes empty
    ...
    if(singleQueue.front()->freq.getFrequency() <= mergeQueue.front()->freq.getFrequency()){
      // if singleQueue has smaller elements at the front or there is a tie
      smallest = singleQueue.front();
      singleQueue.pop();
    }

    else{
      // if mergeQueue has smaller elements
      smallest = mergeQueue.front();
      mergeQueue.pop();
    }
    return smallest;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while((singleQueue.size() + mergeQueue.size()) > 1){
       ...
       //create a new internal node, whose frequency is the sum of childs' frequencies
       TreeNode* parent = new TreeNode(child1->freq.getFrequency() + child2->freq.getFrequency());
       parent->left = child1;
       parent->right = child2;
     ...
     // step3 assign the last node to root_
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
         ...
         bool currBit = bfile.getNextBit();

         if(currBit == 0){
             current = current->left;
          }

          else{//if currBit is 1
             current = current->right;
          }

          // handle leaf node case
          if(current->left == NULL && current->right == NULL){
            // print char to the stringstream
            ss<<current->freq.getCharacter();

            // put current back to root_
            current = root_;
          }
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
     ...
     */
     ...
     if(current->left == NULL && current->right == NULL){
       ...
       bfile.writeBit((bool)1);
       bfile.writeByte(current->freq.getCharacter());
       return;
     }

     else{
       bfile.writeBit((bool)0);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if(currBit == 1){
       ...
       // read in a char
       char currByte = bfile.getNextByte();
       Frequency freq(currByte, 0);
       TreeNode* newNode = new TreeNode(freq);
       return newNode;
       ...
       //if currBit is 0, create an internal node
