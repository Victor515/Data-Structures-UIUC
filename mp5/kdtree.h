...
class KDTree
{
  ...
  private:
    ...
    /**
    Helper function for findNearestNeighbor, to do the recursion
    **/
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, int left, int right, int order) const;

    /**
    Helper function to recursively build a K-d tree
    **/
    void construct(vector<Point<Dim>> & points, int a, int b, int order);

    /**
    Helper function for construct
    This function will place the k-smallest element in its right position, while
    all elements smaller than median on the left and larger on the right
    **/
    void quickSelect(vector<Point<Dim>> & points, int a, int b, int k, int order);

    /**
    Partition a list based on given pivorindex
    Used in quickSelect
    **/
    int partition(vector<Point<Dim>> & points, int a, int b, int pivotindex, int order);
