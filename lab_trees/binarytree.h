...
class BinaryTree
{
  ...
  private:
    ...
    void isOrderedHelper(Node *subRoot, vector<T>& treeVector) const;

    void printPaths(vector<vector<T> > &paths, Node* subRoot, int & path_num) const;

    void sumDistances(Node* subRoot, int & sum, int level) const;
