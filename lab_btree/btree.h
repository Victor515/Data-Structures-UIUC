...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
  ...
  // check for boundaries, that is, if val is smaller than or equal
  //to the first element or greater or equal than the last element
  if(val < elements[start] || val == elements[start]){
    return start;
  }
