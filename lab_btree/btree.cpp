...
{
    /* TODO Your code goes hereThe insertion_idx Function! */
    ...
    if(subroot->elements[first_larger_idx] == key && first_larger_idx < subroot->elements.size()){
      // return the value if found(needs to check the index is not out of bound)
      return subroot->elements[first_larger_idx].value;
    }

    else{
      if(subroot->is_leaf){
        // if subroot is a leaf node, we did not find a match
        return V();
      }

      else{
        // if subroot is not a leaf node, we go to its relevant child
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // insert a new pointer in parent's children
    parent->children.insert(child_itr, new_right);

    // insert the middle element in child into parent's element list
    parent->elements.insert(elem_itr, *mid_elem_itr);

    // copy things to new right, remove things from the new left
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(subroot->elements[first_larger_idx] == pair && first_larger_idx < subroot->elements.size()){
      // if the pair already exists, do nothing
      return;
    }

    if(!subroot->is_leaf){
      // if subroot is not a leaf node
      insert(subroot->children[first_larger_idx], pair);

      if(subroot->children[first_larger_idx]->elements.size() >= order){
        // if the number of elements in child exceeds the order
        split_child(subroot, first_larger_idx);
      }
    }

    else{
      // if subroot is a leaf node
      auto insertPoint = subroot->elements.begin() + first_larger_idx;
      subroot->elements.insert(insertPoint, pair);
